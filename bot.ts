...
export class Bot {
  ...
  private async swap(
    ...
    const txResult = await this.txExecutor.executeAndConfirm(transaction, wallet, latestBlockhash);
    ...
    if (txResult.confirmed) {
      ...
      const history: any[] = JSON.parse(fs.readFileSync(historyFile, 'utf8'));
      ...
      if (direction === 'buy') {
        ...
        const buyData = {
          ...
          computedBuyPrice: (Number(amountIn.toFixed()) / Number(computedAmountOut.amountOut.toFixed())).toFixed(30),
        ...
        const sellData: any = history.find((data: any) => data.mint === tokenIn.mint.toBase58());
        if (sellData) {
          ...
          // sellData.sellPrice = (Number(computedAmountOut.amountOut.toFixed()) / Number(amountIn.toFixed())).toFixed(20);
          ...
          // get index by mint and replace
          const index = history.findIndex(item => item.mint === tokenIn.mint.toBase58());
          history[index] = sellData;
        }
      }
